    ...
    <main className="min-h-screen py-12 px-6 sm:px-12 lg:px-24">
      <section className="max-w-5xl mx-auto">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-xs font-medium text-primary">{s.step}</div>
                  </div>
                  <h3 id={`step-${s.id}-title`} className="mt-2 text-lg font-semibold text-primary">{s.title}</h3>
                  <p className="mt-2 text-sm text-gray-700">{s.desc}</p>
                </div>
              </div>

              <div className="mt-4 flex items-center justify-between">
                <div className="text-sm text-gray-500">Duración estimada: <strong>~3–10 min</strong></div>
              </div>
            </article>
          ))}
        </div>
        ...
        <section className="mt-10 border border-gray-100 rounded-2xl p-6 green-interactive">
          <h4 className="text-base font-semibold text-primary">Usuarios Objetivo</h4>
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 rounded-lg border border-primary green-interactive">
              ...
              <p className="text-xs text-gray-700 mt-1">Diagnósticos estratégicos para planificación de transformación.</p>
            </div>
            <div className="p-4 rounded-lg border border-primary green-interactive">
              <p className="text-sm font-medium text-primary">Estudiantes</p>
              <p className="text-xs text-gray-700 mt-1">Herramienta práctica para aprendizaje y proyectos académicos.</p>
            </div>
            <div className="p-4 rounded-lg border border-primary green-interactive">
              <p className="text-sm font-medium text-primary">Consultores</p>
              <p className="text-xs text-gray-700 mt-1">Herramienta estructurada para trabajo con clientes y evaluaciones.</p>
            </div>
          </div>
        </section>

        <footer className="mt-12 text-sm text-gray-600 border-t pt-6">
          <p><strong>Contacto:</strong> <a href="mailto:proyectogestionti@gmail.com" className=" font-bold text-blue-500 hover:underline">proyectogestionti@gmail.com</a></p>
          <p className="mt-1 text-gray-700"><strong>Créditos:</strong> Jhon Fredy Hoyos Cárdenas, Daniel Ramírez Cárdenas, Hellen Jakeline Rubio</p>
          <p className="mt-1 text-gray-700 italic">Este proyecto se desarrolla como parte de la iniciativa de investigación <strong>&ldquo;Marco de Gobernanza de Datos para Universidades.&rdquo;</strong> IP: Gina Maestre.</p>
        </footer>
      </section>
    </main>
  );
