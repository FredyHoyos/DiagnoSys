...
import { Loader2, ArrowLeft, PlayCircle, CheckCircle, ZoomIn } from "lucide-react";
...
export default function ZoomInPage() {
    ...
    const fetchZoomData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/organization/reports/${reportId}/zoom-in`);
            
            if (!response.ok) {
                throw new Error('Failed to fetch zoom-in data');
            }
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="flex items-center space-x-2">
                    ...
                    <span>Loading zoom-in assessment...</span>
                </div>
            </div>
        );
    }
            ...
            <div className="container mx-auto px-4 py-8">
                <Card>
                    <CardContent className="py-12 text-center">
                        <p className="text-gray-500">Zoom-in data not found</p>
                        <Button 
                            className="mt-4" 
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 mb-2 flex items-center">
                            <ZoomIn className="h-8 w-8 mr-3 text-blue-600" />
                            Zoom In Assessment
                        </h1>
                        <p className="text-gray-600">
                            {zoomData.reportName} v{zoomData.version} - Detailed internal capabilities evaluation
                        </p>
                    </div>
                ...
                <Card>
                    <CardContent className="py-12 text-center">
                        <div className="text-gray-500 mb-4">
                            ...
                            <h3 className="text-lg font-medium mb-2">No zoom-in forms available</h3>
                            <p>There are currently no detailed assessment forms available for this report</p>
                        </div>
                    </CardContent>
                </Card>
            ) : (
